...
file mkdir [file dirname $chatmenu_teama]
file mkdir [file dirname $chatmenu_teamb]
file mkdir [file dirname $chatmenu_spect]
file mkdir [file dirname $binds_install]
file mkdir [file dirname $binds_teama_install]
file mkdir [file dirname $binds_teamb_install]
file mkdir [file dirname $binds_spect_install]
...
foreach {fh tmpl
    tmpl_prefix 
    tmpl_suffix}  [list $chatTeamA   menu_alien_tmpl	 tmpl_a_prefix tmpl_a_suffix \
		       $chatTeamB    menu_human_tmpl	 tmpl_b_prefix tmpl_b_suffix \
		       $chatTeamSpec menu_spectator_tmpl tmpl_s_prefix tmpl_s_suffix] {
    set template [eval "set $tmpl"]
    set menu_prefix [eval "set $tmpl_prefix"]
    set menu_suffix [eval "set $tmpl_suffix"]
	...
	if {$teamMenu != ""} {
	    set text "$text\n      \"$menu_prefix$x. $teamMenu$menu_suffix\""
	}
    }
    puts $fh "    text  $text"
    puts $fh "\}\n"
...
foreach {fh tmpl
	...
	foreach y $bind_keys {
	    ...
	    if {$teamMenu != ""} {
		set text "$text\n      \"$menu_prefix$y. $teamMenu$menu_suffix\""
		set found yes
	    }
	}
	if {!$found} {
